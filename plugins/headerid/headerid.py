...
def register():
    ...
    class HeaderIDPatchedPelicanHTMLTranslator(PelicanHTMLTranslator):
        ...
        def depart_title(self, node):
            ...
            parent = node.parent # gets the section tag where the anchor name is defined
            if isinstance(parent, nodes.section) and parent.hasattr('ids') and parent['ids']:
                ...
                # NOTE : Deals with pages that have the ..contents: directive.
                elif close_tag.startswith('</a></h'):
                    header_text = self.body.pop()
                    header_a_tag = self.body.pop()
                    self.body.append(header_text)
                    self.body.append((header_a_tag + u'\xb6').replace('toc-backref', 'toc-backref headerlink'))
